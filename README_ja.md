...
Apple Watch と CarPlay、および Mac にも対応しました。
...
必要な全てのサイズの PNG を然るべきファイル名で生成します（Contents.json も出力されます）。  
...
あまり手間をかけずにそれっぽくなるので、起動イメージの元画像には 2048×2048 ピクセルの正方形を推奨しています（必須ではありません）。
...
別ディレクトリの jar から起動した場合、状態は引き継がれません。
